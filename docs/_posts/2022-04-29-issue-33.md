...
* NTVPTX tests were updated to use the `ptxas` proprietary compiler to catch errors in the PTX output from llc. The local of the tool can be specified via a CMake option or an environment variable. [D121727](https://reviews.llvm.org/D121727), [D124537](https://reviews.llvm.org/D124537)
